"""
Ingest Pickleball Studio CSV data into the Paddle catalog.
Focuses on core_thickness_mm, power_original, and adding PB Studio to validation sources.
Updates ONLY NULL fields unless explicitly overriding.

Usage:
  docker compose exec backend_v3 python scripts/ingest_pb_studio_csv.py --csv /app/data/pbstudio.csv
  docker compose exec backend_v3 python scripts/ingest_pb_studio_csv.py --csv /app/data/pbstudio.csv --dry-run
"""
import sys
import argparse
...
def ingest(csv_path: str, dry_run: bool = False):
    ...
    print(f"🧠 PB Studio Ingest Pipeline — {csv_path} → PostgreSQL [{mode}]")
    print("=" * 60)
    ...
    core_mm_col = find_column(df, ["core", "thickness"]) or find_column(df, ["mm"])
    power_col = find_column(df, ["power"])
    swing_col = find_column(df, ["swing"])
    twist_col = find_column(df, ["twist"])
    ...
    with Session(sync_engine) as session:
        ...
        for paddle in paddles:
            ...
            updates = {}
            if core_mm_col:
                cm = clean_float(csv_row[core_mm_col])
                if cm and not paddle.core_thickness_mm:
                    updates['core_thickness_mm'] = cm
            
            if power_col:
                pw = clean_float(csv_row[power_col])
                if pw and not paddle.power_original:
                    updates['power_original'] = pw
                    
            if swing_col:
                sw = clean_int(csv_row[swing_col])
                if sw and not paddle.swing_weight:
                    updates['swing_weight'] = sw
                    
            if twist_col:
                tw = clean_float(csv_row[twist_col])
        ...
        print("\n".join(match_log))
        print(f"\n{'=' * 60}")
        print(f"✅ PB STUDIO INGESTION {'PREVIEW' if dry_run else 'COMPLETE'}")
        print(f"   🟢 Updated Paddles: {updated_count}")
        print(f"   🔴 No match:        {no_match}")

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--csv", type=str, required=True, help="Path to PB Studio CSV")
